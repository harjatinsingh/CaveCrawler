...
int main(int argc, char** argv)
{
    ...
    // ros::param::get("/gps_goal/points_outdoor", path_local);
    ...
    // Iterate through vector of waypoints for setting goals
        latiGoal = 43.50089451;
        longiGoal = -80.54642133;
        ...
        ROS_INFO_STREAM("map point"<<map_point);
        ...
        {
            ROS_INFO("Winebot has reached its goal!");
            //switch to next waypoint and repeat
        }
        else
        {
            ROS_ERROR("Winebot was unable to reach its goal. GPS Waypoint unreachable.");
